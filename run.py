                ...
                for i, (x, y, bucket_id, len_x, len_y) in enumerate(dl.iterate_one_epoch()):
                    ...
                    if do_eval and i % FLAGS.eval_frequency == 0:
                        ...
                        for j, (x, y, bucket_id, len_x, len_y) in enumerate(eval_dl.iterate_one_epoch()):
                            ...
                            if eval_using_bleu:
                                preds.extend([utils.transform_for_bleu(si,
                                                                       ...
                                                                       ignore_special=True,
                                                                       delim=config["delimiter"], bpe_used=bpe_used) for
                                              sample in samples for si in sample.sample_id])
                                targets.extend([[utils.transform_for_bleu(yi,
                                                                          vocab=eval_dl.target_idx2seq,
                                                                          ignore_special=True,
                                                                          delim=config["delimiter"], bpe_used=bpe_used)]
                                                for yii in tgt for yi in yii])
                        ...
                        if i > 0:
                            ...
                            epoch_saver.save(sess, save_path=os.path.join(FLAGS.logdir, "model-eval"),
                                             global_step=global_step)
