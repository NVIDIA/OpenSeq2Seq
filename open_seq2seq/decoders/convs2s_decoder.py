...
class ConvS2SDecoder(Decoder):
  ...
  def _decode(self, input_dict):
    ...
    encoder_outputs_b = input_dict['encoder_output'].get('outputs_b', encoder_outputs)

    inputs_attention_bias = input_dict['encoder_output'].get('inputs_attention_bias_cs2s', None)
    ...
    with tf.name_scope("decode"):
      ...
      if len(self.layers) == 0:
        ...
        with tf.variable_scope("embedding"):
          if 'embedding_softmax_layer' in input_dict['encoder_output'] and self.params['shared_embed']:
        ...
        for i in range(self.params['decoder_layers']):
          ...
          att_layer = attention_wn_layer.AttentionLayerNormalized(out_dim, embed_size=self._tgt_emb_size,
                                                                    layer_id=i+1, add_res=True)
        ...
        if not self.params['shared_embed']:
          self.layers.append(ffn_wn_layer.FeedFowardNetworkNormalized(
                                                                    self.params.get("out_emb_size", self._tgt_emb_size),
                                                                    self._tgt_vocab_size,
                                                                    dropout=self.params["out_dropout_keep_prob"],
                                                                    var_scope_name="linear_mapping_to_vocabspace"))
        else:
          # if embedding is shared, the shared embedding is used as the final linear projection to vocab space
          self.layers.append(None)

      if targets is None:
        return self.predict(encoder_outputs, encoder_outputs_b, inputs_attention_bias)
      else:
  ...
  def decode_pass(self, targets, encoder_outputs, encoder_outputs_b, inputs_attention_bias):
    ...
    # Prepare inputs to decoder layers by applying embedding and adding positional encoding.
