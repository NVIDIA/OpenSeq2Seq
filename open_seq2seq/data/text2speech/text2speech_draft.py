...
class Text2SpeechDataLayer(DataLayer):
  ...
  def __init__(self, params, model, num_workers=None, worker_id=None):
    ...
    * **augmentation** (dict) --- optional dictionary with data augmentation
    ...
    for csv in params['dataset_files']:
      files = pd.read_csv(csv, encoding='utf-8', sep='\x7c', header=None, names=names, quoting=3)
    ...
    # NEEDS TO BE CHECKED!!!
    # Also needs a train - val - test split
  ...
  def _parse_audio_transcript_element(self, element):
    ...
    file_path = self.params['dataset_location'] + audio_filename + ".wav"
    ...
    return np.int32(target), \
           np.int32([len(target)]), \
           source.astype(self.params['dtype'].as_numpy_dtype()), \
           np.int32([len(source)])
