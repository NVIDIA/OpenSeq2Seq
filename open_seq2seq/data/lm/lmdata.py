...
from open_seq2seq.data.lm.lmutils import Dictionary, Corpus
...
class LMTextDataLayer(DataLayer):
  ...
  def __init__(self, params, model, num_workers=1, worker_id=0):
    ...
    self._processed_data_folder = self.params.get('processed_data_folder', 'wkt_processed_data')
...
class IMDBDataLayer(DataLayer):
  ...
  def __init__(self, params, model, num_workers=1, worker_id=0):
    ...
    self._processed_data_folder = self.params.get('processed_data_folder', 'imdb_processed_data')
    self._data_root = self.params.get('data_root', None)
    self.lm_vocab = Dictionary()
    self.lm_vocab.load_vocab(self.params[lm_vocab_file])
