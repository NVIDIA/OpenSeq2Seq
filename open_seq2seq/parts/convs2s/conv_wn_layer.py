...
def gated_linear_units(inputs):
  ...
  Args:
    inputs: A float32 tensor with shape [batch_size, length, 2*out_dim]
  Returns:
...
class Conv1DNetworkNormalized(tf.layers.Layer):
  ...
  def __init__(self, in_dim, out_dim, kernel_width, mode, layer_id,
    ...
    Args:
      ...
      decode_padding: bool specifies if this convolution layer is in decoder
          or not in decoder padding is done explicitly before convolution
    """
    ...
    with tf.variable_scope("conv_layer_" + str(layer_id)):
      ...
      self.v_norm = tf.norm(self.vars.initialized_value(), axis=[0, 1])
      ...
      self.W = tf.reshape(self.g, [1, 1, 2 * out_dim]) * \
               tf.nn.l2_normalize(self.vars, [0, 1])
  ...
  def call(self, inputs):
    ...
    output = gated_linear_units(output)
