...
:class:`DataLayer <open_seq2seq.data.data_layer.DataLayer>`
...
0. Static methods:
   :func:`get_required_params() <open_seq2seq.data.data_layer.DataLayer.get_required_params>` and
   :func:`get_optional_params() <open_seq2seq.data.data_layer.DataLayer.get_optional_params>`
   to specify required and optional parameters correspondingly

1. :meth:`__init__(self, params, num_workers=None, worker_id=None) <open_seq2seq.data.data_layer.DataLayer.__init__>`:
...
2. :meth:`gen_input_tensors(self) <open_seq2seq.data.data_layer.DataLayer.gen_input_tensors>`:
...
3. :meth:`next_batch_feed_dict(self) <open_seq2seq.data.data_layer.DataLayer.next_batch_feed_dict>`:

   -  Most likely should return empty Python dictionary if you are using
      ``tf.data``.
   -  If you are using Placeholders in :meth:`gen_input_tensors(self) <open_seq2seq.data.data_layer.DataLayer.gen_input_tensors>`,
      this method should return corresponding feed\_dictionary.

4. :meth:`shuffle(self) <open_seq2seq.data.data_layer.DataLayer.shuffle>`:

   -  If you are not using ``tf.data`` this method should shuffle your dataset.

5. :meth:`get_size_in_samples(self) <open_seq2seq.data.data_layer.DataLayer.get_size_in_samples>`:
